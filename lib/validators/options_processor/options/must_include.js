...
class MustInclude {
  static process (validatorContext, input, rule) {
    ...
    if (!on_success) on_success = `${validatorContext.name} ${filter.all ? 'all' : ''}must include '${regex}'`
    if (!on_failure) on_failure = `${validatorContext.name} ${filter.all ? 'all' : ''}does not include "${regex}"`
    let regexObj
