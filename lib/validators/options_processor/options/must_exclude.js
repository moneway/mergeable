...
class MustExclude {
  static process (validatorContext, input, rule) {
    ...
    if (!on_success) on_success = `${validatorContext.name} ${filter.all ? 'all' : ''}must exclude '${regex}'`
    if (!on_failure) on_failure = `${validatorContext.name} ${filter.all ? 'all' : ''}included "${regex}"`
    let regexObj
