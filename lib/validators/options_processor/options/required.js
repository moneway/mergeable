...
class Required {
  static process (validatorContext, input, rule) {
    ...
    if (!on_success) on_success = `${validatorContext.name}: all required reviewers have approved`
    if (!on_failure) on_failure = `${validatorContext.name}: ${requiredReviewers}required`
